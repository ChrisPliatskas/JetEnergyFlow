...
//Fill the measured, missed and Fake distribution at each level and create the response object
    ...
    TFile *fout = new TFile(Form("ResponseMatrix_PbPb_ConstSub_dRmax005_LCJC8_wFakes_smallR_NewBinW%d_Ptw%d_ptmin%d_%d_Dptmin%d_%d.root",BinWidth,binWidthPt,int(PT_MIN),int(pt_max),int(DPT_MIN),int(DPT_MAX)),"UPDATE");
    RM_thn->Write();
