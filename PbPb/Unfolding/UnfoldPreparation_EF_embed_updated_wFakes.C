void UnfoldPreparation_EF_embed_updated_wFakes(TString strIn="AnalysisResults.root",Double_t Rjet =0.1,Double_t PT_MIN = 0.0, Double_t DPT_MIN = -20.5,Double_t DPT_MAX=30.5,Int_t BinWidth=3,Int_t binWidthPt=5,TString strL = "AliAnalysisTaskEmcalJetEnergyFlow_PicoTracks_caloClusters_emcalCells_Embed_ConstSub_SmallR_histos", Int_t skipBins = 0){
...
Double_t pt_max = 140.0;  //Upper Pt limit for detector level (Gen level is higher by 10 GeV (feed-in effect)).
Double_t pt_min_gen = 0;//PT_MIN-5.;// Lower Pt limit for generator level
...
Double_t Dpt_max_gen = DPT_MAX+2*BinWidth;//31.5; //Upper Dpt limit for truth level(Gen level is higher by 5 GeV (feed-in effect))

      //Detector, Particle level
Int_t nPtBinWidth[2] = {binWidthPt,10};
Double_t ptmin[2] = {pt_min,pt_min_gen};
Double_t ptmax[2] = {pt_max, 200};//pt_max+10};
...
Double_t Dptmin[2] = {Dpt_min ,-double(BinWidth)/2};
...
//Fill the measured, missed and Fake distribution at each level and create the response object
    ...
    TFile *fout = new TFile(Form("ResponseMatrix_PbPb_ConstSub_Updated_LCJC8_wFakes_smallR_NewBinW%d_Ptw%d_ptmin%d_Dptmin%d_%d.root",BinWidth,binWidthPt,int(PT_MIN),int(DPT_MIN),int(DPT_MAX)),"UPDATE");
    RM_thn->Write();
