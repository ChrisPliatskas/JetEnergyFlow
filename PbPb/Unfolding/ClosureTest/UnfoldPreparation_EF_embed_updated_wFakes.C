...
Double_t pt_max = 120.0;//140.0;  //Upper Pt limit for detector level (Gen level is higher by 10 GeV (feed-in effect)).
Double_t pt_min_gen = 0.;//0;//PT_MIN-5.;// Lower Pt limit for generator level
...
Double_t Dpt_max_gen = 31.5; //Upper Dpt limit for truth level(Gen level is higher by 5 GeV (feed-in effect))
...
//Fill the measured, missed and Fake distribution at each level and create the response object
    ...
    TFile *fout = new TFile(Form("ResponseMatrix_PbPb_ConstSub_Updated_LCJC_wFakes_smallR_NewBinW%d_LC8_Ptw%d_ptmin%d_Dptmin%d_%d.root",BinWidth,binWidthPt,int(PT_MIN),int(DPT_MIN),int(DPT_MAX)),"UPDATE");
    RM_thn->Write();
