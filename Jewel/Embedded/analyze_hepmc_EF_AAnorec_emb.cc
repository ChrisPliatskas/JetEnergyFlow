...
static const int do_bkg = 1;//Enable when working with the Recoil sample or embedding
...
for (Int_t iR=0; iR<nR; iR++){
	Float_t Rjet = (iR+1)*Rstep;
        ...
        histname = TString::Format("hJetPt_he_R%03d",int(Rjet*100));
        htitle = TString::Format("Jet p_{t} spectrum for R=%.2f (Hard event);p_{t} (GeV/c)",Rjet);
        hJetPt_he[iR] = new TH1F(histname,htitle,NPtBins,PtBin_min,PtBin_max);

        histname = TString::Format("hJetEta_he_R%03d",int(Rjet*100));
        htitle = TString::Format("Jet #eta distribution for R=%.2f (Hard event);#eta",Rjet);
        hJetEta_he[iR] = new TH1F(histname,htitle,NEtaBins,-EtaBin_max,EtaBin_max);

        histname = TString::Format("hJetPtJES_R%03d",int(Rjet*100));
        htitle = TString::Format("Jet p_{t} vs jet energy resolution for R=%.2f;p_{t} (GeV/c)",Rjet);
        hPtJES[iR] = new TH2F(histname,htitle,NPtBins,PtBin_min,PtBin_max,100,-1,1);
	...
	hDptPtMultiplicity[iR]= new TH3F(histname,htitle,NDptBins,DPtBin_min,DPtBin_max,NPtBins,PtBin_min,PtBin_max,40,0,40);
        ...
        int nThermalParticles = 4000;
          TF1* f_pT = new TF1("f_pT","x*exp(-x/0.6)", 0.0, 400.0);
          f_pT->SetNpx(40000);
...
if (debug)cout << "Event: " <<Event_count<<endl;
  ...
  hNEvent->Fill(0.5,evt->weights()[0]); // count events
    ...
    std::vector <fastjet::PseudoJet> Embedded_tracks;
                ...
                for(int part = 0; part < nThermalParticles; part++)
                  {
                        ...
                        ThermalParticle.set_user_index(index);
                        Embedded_tracks.push_back(ThermalParticle);
       //               Thermal_sumPt+=ThermalParticle.pt();
                           }
...
std::vector <fastjet::PseudoJet> Corrected_tracks;
        ...
        bge.set_particles(Embedded_tracks);
        Corrected_tracks = subtractor.subtract_event(Embedded_tracks);
                }
...
//	 cout<<"Number of corrected event tracks: "<<Corrected_tracks.size()<<endl;
	...
	clustSeqCh_HE[iR]=new fastjet::ClusterSequenceArea(Tracks_in, jetDefCh,areaDef);
	...
	for (auto k:AcceptedJets[iR]){
        ...
        for (auto k:jets_HE[iR]){
                hJetPt_he[iR]->Fill(k.pt(),evt->weights()[0]);
                hJetEta_he[iR]->Fill(k.eta(),evt->weights()[0]);
                                     }  }
...
//Jet energy resolution calculation	

        iHardIndex.Set(jets[iR].size());
        iEmbIndex.Set(jets_HE[iR].size());

	JetMatcher(&jets_HE[iR],kLowRJets,&jets[iR],kHighRJets,iHardIndex,iEmbIndex,0,Maxdist);

	for(Int_t j=0; j<iEmbIndex.GetSize();j++){
		if(iEmbIndex[j]>=0){
			Int_t match_jes = iHighRIndex[j];
		if(iHardIndex[match_jes]==j){
		hPtJES[iR]->Fill(jets_HE[iR].at(j).pt(),(jets[iR].at(match_jes).pt()-jets_HE[iR].at(j).pt())/jets_HE[iR].at(j).pt(),evt->weights()[0]);	
...
/*		
	...
	delete clustSeqCh_HE[iR];
      	clustSeqCh[iR]=0;clustSeqCh_HE[iR]=0;
   	 }
