//--------------------------------Script description------------------------------------------------
//Date: 21st Nov 2024 Developer: Chris Pliatskas
// This is the analysis script for the measurement of jet energy flow fluctuations at JEWEL vacuum simulations. These model predictions can be compared to the respective measurement with the experimental pp data. There has been an effort to maintain the same analysis selection criteria and cuts as in the corresponding AliPhysics task. The main output of this analysis script are the multidimensional histograms which can then be further analysed and viewd with the corresponding plotter. This script has been developed for the analysis of JEWEL vacuum simulations and shares common elements with the analysis scripts for the JEWEL AA scenarios. Future development could merge the distinct analysis scripts to a multi-purpose JEWEL analysis code. 

//------------------------------Including necessary libraries-------------------------------------------------//
// Required to handle the hepmc format of the JEWEL simulations
#include "HepMC/IO_GenEvent.h"
#include "HepMC/GenEvent.h"
// Fastjet libraries required for clustering and possible background subtraction
...
// Generally usefull ROOT libraries
#include "TPDGCode.h"
#include "TFile.h"
#include "TH1.h"
#include "TH2.h"
...
//-----------------------------------------Secondary function (legacy) to check if the particle is stable----//
...
//---------------Secondary function to impose the geometrical matching in the eta-phi plane of the narrow and wide jet participaing in the energy flow measurement. The implementation has been inheritted and adjusted from the correponding AliPhysics task. Matching is purely geometrical, bijective with maximum cutoff distance. The same function can be applied for a matching of particle to detector level jets. The arguments include the two jet collections as vectors, the size of the jet populations, arrays which point to the index of the matched jet and a maximum distance cutoff. 
