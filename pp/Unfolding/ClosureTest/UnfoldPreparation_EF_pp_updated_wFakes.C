void UnfoldPreparation_EF_pp_updated_wFakes(TString strIn="AnalysisResults.root",Double_t Rjet =0.1,Double_t PT_MIN = 0.0, Double_t DPT_MIN = -20.5,Double_t DPT_MAX=30.5,Int_t BinWidth=3,Int_t binWidthPt=5,TString strL = "AliAnalysisTaskEmcalJetEnergyFlow_tracks_caloClusters_emcalCells_ppMC_GenDet_histos", Int_t skipBins = 0){
...
Double_t pt_min_gen = 10;//PT_MIN-5.;// Lower Pt limit for generator level
...
Int_t nPtBinWidth[2] = {binWidthPt,5};
...
Double_t Dptmax[2] = {Dpt_max,Dpt_max+BinWidth};//31.5,};//Dpt_max+BinWidth};
...
//Fill the measured, missed and Fake distribution at each level and create the response object
    ...
    TFile *fout = new TFile(Form("ResponseMatrix_pp_PrelimConfig_wFakes_NewBinW%d_Ptw%d_ptmin%d_Dptmin%d_%d.root",BinWidth,binWidthPt,int(PT_MIN),int(DPT_MIN),int(DPT_MAX)),"UPDATE");
    RM_thn->Write();
