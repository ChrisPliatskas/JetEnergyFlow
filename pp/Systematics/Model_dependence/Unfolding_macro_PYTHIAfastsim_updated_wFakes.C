...
void Unfolding_macro_PYTHIAfastsim_updated_wFakes(TString DatafileName = "AnalysisResults.root",TString ResponsefileName = "ResponseMatrix_ppMC_ptmin0.root",Double_t Rjet = 0.1, Int_t Ptlow =0 , Int_t Pthigh = 100, Int_t BinWidth=3, Int_t Niter = 1){
...
THnSparse* ResponseMatrix = static_cast<THnSparse*>(ResponseFile->Get(Form("RM_THN%03d",int(Rjet*100))));
...
//Ratios
        ...
        //Saving the output
        TString Outputname = Form("UnfoldedResults_pp_PYTHIAfastSim_wFakes_smallR_Binw%d_pt_%d_%d_ptmin%d_Dptmin%d_max%d_Iter%d.root",BinWidth,Ptlow,Pthigh,int(ptmin),int(Dptmin),int(Dptmax),Niter);
