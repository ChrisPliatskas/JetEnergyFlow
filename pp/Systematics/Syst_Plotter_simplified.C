...
TFile*fTrackEff = TFile::Open(Form("Systematic_%d_%d_pp_TrackEff.root",ptlow,pthigh));
...
TFile*f_Prior_mod1 = TFile::Open(Form("Systematic_%d_%d_pp_PriorScalingDPt5percent_mod1.root",ptlow,pthigh));
...
TFile*f_Prior_mod2 = TFile::Open(Form("Systematic_%d_%d_pp_PriorScalingDPt5percent_mod2.root",ptlow,pthigh));
...
TFile*f_N3 = TFile::Open(Form("Systematic_%d_%d_pp_Niter3.root",ptlow,pthigh));
TH1D*pro_N3=(TH1D*) f_N3->Get(Form("pro_sys_R%03d",R));
TH1D*Ratio_N3=(TH1D*) f_N3->Get(Form("Systematic_Ratio_R%03d",R));
TH1D*Var_N3=(TH1D*) f_N3->Get(Form("Systematic_variation_R%03d",R));

TFile*f_N5 = TFile::Open(Form("Systematic_%d_%d_pp_Niter5.root",ptlow,pthigh));
TH1D*pro_N5=(TH1D*) f_N5->Get(Form("pro_sys_R%03d",R));
TH1D*Ratio_N5=(TH1D*) f_N5->Get(Form("Systematic_Ratio_R%03d",R));
TH1D*Var_N5=(TH1D*) f_N5->Get(Form("Systematic_variation_R%03d",R));

TFile*f_trunc = TFile::Open(Form("Systematic_%d_%d_pp_Truncation.root",ptlow,pthigh));
...
TFile*f_model = TFile::Open(Form("Systematic_%d_%d_pp_ModelDependence.root",ptlow,pthigh));
TH1D*pro_model=(TH1D*) f_model->Get(Form("pro_sys_R%03d",R));
TH1D*Ratio_model=(TH1D*) f_model->Get(Form("Systematic_Ratio_R%03d",R));
TH1D*Var_model=(TH1D*) f_model->Get(Form("Systematic_variation_R%03d",R));

TH1D* Ratio_Niter = (TH1D*)Ratio_model->Clone(); Ratio_Niter->Reset();
TH1D* Ratio_Prior = (TH1D*)Ratio_model->Clone(); Ratio_Prior->Reset();

Int_t Nbins = Ratio_N3->GetNbinsX();
double N3[11] = {}; double N5[11] = {};
...
for(int i=1;i<=Nbins;i++){
        ...
        N3[i-1] = Ratio_N3->GetBinContent(i);
        N5[i-1] = Ratio_N5->GetBinContent(i);
        Prior[i-1] = abs(prior1[i-1] - prior2[i-1])/2;
        Niter[i-1] = abs(N5[i-1] - N3[i-1])/2;
                        }
...
Ratio_model->SetLineColor(colors[4]); Ratio_Prior->SetLineColor(colors[5]);

Ratio_stat->SetLineWidth(3);Ratio_trackeff->SetLineWidth(3);
Ratio_Niter->SetLineWidth(3);Ratio_trunc->SetLineWidth(3);
Ratio_Prior->SetLineWidth(3);Ratio_model->SetLineWidth(3);
...
TPad *pad1=new TPad("pad1","pad1",0.,0.,1.,1.);
 ...
 Ratio_Prior->Draw("same"); Ratio_model->Draw("same");
 l11->Draw("same");
  ...
  leg1->AddEntry(Ratio_model,"Model dependence","l");
  leg1->Draw("same");

  lat.DrawLatex(.2,.84,"This thesis");
  lat.DrawLatex(.2,.79,"ALICE pp #sqrt{#it{s}} =13 TeV");
