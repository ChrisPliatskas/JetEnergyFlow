void Prior_Scaling_Dpt_pp_FinalSolution(const char* fname,double Rjet,Double_t PT_MIN = 0.0, Double_t DPT_MIN = -20.0,Double_t PT_MAX=100.0,Double_t DPT_MAX=30.0, double BinWidth = 3.,double perc = 0.05){
TFile*f = TFile::Open(fname);
AliEmcalList* ali;
f->GetObject("AliAnalysisTaskEmcalJetEnergyFlow_tracks_caloClusters_emcalCells_ppMC_GenDet_histos",ali);
...
Double_t pt_max = 100.0;//40.0;  //Upper Pt limit for detector level (Gen level is higher by 10 GeV (feed-in effect)).
...
Double_t ptmax[2] = {pt_max,pt_max+5.};// 200};
...
TFile*fout = new TFile(Form("Modified_Response_pp_FinalSolution_BinW%d_Dpt%d_DptMin%02d_DptMax%02d_2024_test.root",int(BinWidth),int(perc*100),int(Dpt_min),int(Dpt_max)),"UPDATE");
